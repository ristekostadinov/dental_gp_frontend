...
import { PatientDTO, Patient, PatientRequest } from '../domains/Patient';
...
export class PatientService {
  ...
  save(body: PatientRequest){
    return this._httpClient.post<PatientRequest>(`${environment.apiUrl}api/patients`,body)
  }
  ...
  findById(id: string): Observable<Patient>{
    return this._httpClient.get<Patient>(`${environment.apiUrl}api/patients/${id}`)
  }

  edit(id: string, body: PatientRequest){
    return this._httpClient.put<PatientRequest>(`${environment.apiUrl}api/patients/${id}`, body);
  }
